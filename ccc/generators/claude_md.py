...
class ClaudeMdEnhancer:
    ...
    def generate_enhanced_llm_md(self, project: ProjectInfo) -> str:
        ...
        lines.append(">")
        lines.append("> **For AI agents**: Read `.llm-context/context-manifest.json` first.")
        lines.append("> It lists all available artifacts with token estimates and guidance")
        lines.append("> on which to include for different task types.")
        lines.append("")
        ...
        # Known Gaps — auto-populated from source TODOs and alignment data
        gaps = self._extract_known_gaps()
        lines.append("")
        lines.append("## Known Gaps")
        lines.append("<!-- Auto-populated from source TODOs and ccc align. Edit freely. -->")
        lines.append("<!-- This section helps AI avoid hallucinating unimplemented features. -->")
        if gaps:
            for gap in gaps:
                lines.append(f"- {gap}")
        else:
            lines.append("- TODO: List known missing features, incomplete implementations, or areas to avoid")

        lines.append("")
        lines.append("## Generated Context")
        lines.append("")
        lines.append("See `.llm-context/` for auto-extracted context:")
        lines.append("- `context-manifest.json` — artifact index with token estimates (read this first)")
        ...
        lines.append("")
        lines.append("## For the AI Assistant")
        lines.append("")
        lines.append("After completing a task using this context, please append to")
        lines.append("`.llm-context/ai-observations.md` (create if missing):")
        lines.append("")
        lines.append("```")
        lines.append("## YYYY-MM-DD — <task description>")
        lines.append("")
        lines.append("**Context used**: <which artifacts were helpful>")
        lines.append("**Sufficient**: <yes / partial / no>")
        lines.append("**Missing**: <what information was absent or unclear>")
        lines.append("**Assumptions made**: <what you had to guess due to missing context>")
        lines.append("**Files needed not referenced**: <any files you opened not in LLM.md>")
        lines.append("```")
        lines.append("")
        lines.append("These observations improve context quality over time.")
        lines.append("")
    ...
    def _extract_known_gaps(self) -> list:
        """
        Scan source files for TODO/FIXME/HACK/NotImplemented markers and
        read any previous ccc align output to populate ## Known Gaps.

        Returns a list of gap strings (without leading '- ').
        Capped at 20 entries to keep LLM.md readable.
        """
        import re
        from ..utils.files import safe_read_text, should_skip_path, EXCLUDE_DIRS

        gaps = []
        seen = set()

        # Source code TODOs — look in non-test Python/TS/Go/Rust files
        todo_pattern = re.compile(
            r"#\s*(TODO|FIXME|HACK|XXX)[:\s]+(.+)|"
            r"//\s*(TODO|FIXME|HACK|XXX)[:\s]+(.+)",
            re.IGNORECASE,
        )
        not_implemented_py = re.compile(r"raise\s+NotImplementedError\s*\(([^)]*)\)")

        src_exts = {".py", ".ts", ".tsx", ".js", ".go", ".rs"}
        for src_file in sorted(self.root.rglob("*")):
            ...
            if ".test." in src_file.name or ".spec." in src_file.name:
                continue
            if "test" in src_file.parts or "tests" in src_file.parts:
                continue
            ...
            for m in todo_pattern.finditer(content):
                # Groups: (py_kind, py_text, ts_kind, ts_text)
                text = (m.group(2) or m.group(4) or "").strip()
                text = text[:120]  # truncate very long ones
                if text and text not in seen:
                    seen.add(text)
                    rel = str(src_file.relative_to(self.root))
                    line = content[:m.start()].count("\n") + 1
                    gaps.append(f"{text} (`{rel}:{line}`)")
                    if len(gaps) >= 15:
                        break
            ...
            for m in not_implemented_py.finditer(content):
                msg = m.group(1).strip().strip("'\"") or "NotImplemented"
                if msg not in seen:
                    seen.add(msg)
                    rel = str(src_file.relative_to(self.root))
                    line = content[:m.start()].count("\n") + 1
                    gaps.append(f"Not yet implemented: {msg} (`{rel}:{line}`)")

        # Read from ccc align output if it exists
        align_file = self.root / ".llm-context" / "alignment-report.json"
        if align_file.exists():
            try:
                import json
                data = json.loads(safe_read_text(align_file) or "{}")
                for issue in data.get("issues", []):
                    if issue.get("kind") == "missing_impl":
                        msg = issue.get("message", "")
                        if msg and msg not in seen:
                            seen.add(msg)
                            gaps.append(f"[align] {msg}")
            except Exception:
                pass
